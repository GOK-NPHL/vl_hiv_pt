...
class Application_Service_Reports {
    ...
    public function getAllShipments($parameters) {
        ...
        $sQuery = $dbAdapter->select()->from(array('s' => 'shipment'))
                ...
                ->joinLeft(array('sp' => 'shipment_participant_map'), 'sp.shipment_id=s.shipment_id', array('report_generated', 'participant_count' => new Zend_Db_Expr('count("participant_id")'), 'reported_count' => new Zend_Db_Expr("SUM(shipment_test_date <> null)"), 'reported_percentage' => new Zend_Db_Expr("ROUND((SUM(shipment_test_date <> null)/count('participant_id'))*100,2)"), 'number_passed' => new Zend_Db_Expr("SUM(final_result = 1)")))
    ...
    public function getParticipantPerformanceReport($parameters) {
        ...
        $aColumns = array(
            ...
            new Zend_Db_Expr("SUM(sp.shipment_test_date <> null)"),
            new Zend_Db_Expr("(SUM(sp.shipment_test_date <> null) - SUM(is_excluded = 'yes'))"),
        ...
        $sQuery = $dbAdapter->select()->from(array('s' => 'shipment'))
                ...
                ->joinLeft(array('sp' => 'shipment_participant_map'), 'sp.shipment_id=s.shipment_id',
						   array("DATE_FORMAT(s.shipment_date,'%d-%b-%Y')",
								 ...
								 "total_responses" => new Zend_Db_Expr("SUM(sp.shipment_test_date <> null)"),
								 "valid_responses" => new Zend_Db_Expr("(SUM(sp.shipment_test_date <> null) - SUM(is_excluded = 'yes'))"),
    ...
    public function getParticipantPerformanceReportByShipmentId($shipmentId) {
        ...
        $sQuery = $dbAdapter->select()->from(array('s' => 'shipment'))
                ->join(array('sl' => 'schemes'), 's.scheme_type=sl.scheme_id')
                ->joinLeft(array('sp' => 'shipment_participant_map'), 'sp.shipment_id=s.shipment_id', array("DATE_FORMAT(s.shipment_date,'%d-%b-%Y')", "total_shipped" => new Zend_Db_Expr('count("sp.map_id")'), "total_responses" => new Zend_Db_Expr("SUM(sp.shipment_test_date <> null)"),
                    "valid_responses" => new Zend_Db_Expr("(SUM(sp.shipment_test_date <> null) - SUM(is_excluded = 'yes'))"),
                    ))
    ...
    public function getShipmentResponseReport($parameters) {
        ...
        if($parameters['scheme']=='vl'){
            $sQuery = $dbAdapter->select()->from(array('s' => 'shipment'), array('shipment_code'))
                ->join(array('sl' => 'schemes'), 's.scheme_type=sl.scheme_id')
                ->join(array('sp' => 'shipment_participant_map'), 'sp.shipment_id=s.shipment_id', array("total_responses" => new Zend_Db_Expr("SUM(sp.shipment_test_date <> null)"), "total_passed" => new Zend_Db_Expr("SUM(sp.final_result=1)"), "valid_responses" => new Zend_Db_Expr("(SUM(sp.shipment_test_date <> null) - SUM(is_excluded = 'yes'))")))
    ...
    public function getShipmentResponseCount($shipmentId, $date, $step = 5, $maxDays = 60) {
        ...
        for ($i = $step; $i <= $maxDays; $i+=$step) {
            ...
            $sQuery = $dbAdapter->select()->from(array('s' => 'shipment'), array(''))
                    ->joinLeft(array('sp' => 'shipment_participant_map'), 'sp.shipment_id=s.shipment_id', array('reported_count' => new Zend_Db_Expr("SUM(shipment_test_date <> null)")))
                    ->where("s.shipment_id = ?", $shipmentId)
                    ->group('s.shipment_id');
    ...
    public function getParticipantPerformanceRegionWiseReport($parameters) {
        ...
        $sQuery = $dbAdapter->select()->from(array('s' => 'shipment'))
                ->join(array('sl' => 'schemes'), 's.scheme_type=sl.scheme_id')
                ->joinLeft(array('sp' => 'shipment_participant_map'), 'sp.shipment_id=s.shipment_id', array("DATE_FORMAT(s.shipment_date,'%d-%b-%Y')", "total_shipped" => new Zend_Db_Expr('count("sp.map_id")'), "total_responses" => new Zend_Db_Expr("SUM(sp.shipment_test_date <> null)"), "valid_responses" => new Zend_Db_Expr("(SUM(sp.shipment_test_date <> null) - SUM(is_excluded = 'yes'))"), "total_passed" => new Zend_Db_Expr("(SUM(final_result = 1))"), "pass_percentage" => new Zend_Db_Expr("((SUM(final_result = 1))/(SUM(final_result = 1) + SUM(final_result = 2)))*100"), "average_score" => new Zend_Db_Expr("((SUM(Case When sp.is_excluded='yes' Then 0 Else sp.shipment_score End)+SUM(Case When sp.is_excluded='yes' Then 0 Else sp.documentation_score End))/(SUM(final_result = 1) + SUM(final_result = 2)))")))
